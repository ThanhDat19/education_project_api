...
@section('contents')
    <form action="" method="post">
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" class="form-control" value="{{ $lesson->title }}" placeholder="Nhập tiêu đề bài học">
        </div>
        <div class="form-group">
            <label for="lesson">Video url</label>
            <input type="text" name="video_url" class="form-control"  value="{{ $lesson->video_url }}" placeholder="Nhập video url">
        </div>
        <div class="form-group">
            <label for="lesson">Miêu tả</label>
            <textarea name="short_text" class="form-control"> {{ $lesson->short_text }}</textarea>
        </div>
        <div class="form-group">
            <label for="lesson">Nội dung</label>
            <textarea name="full_text" id="content" class="form-control">{{ $lesson->full_text }}</textarea>
        </div>
        <div class="form-group">
            <label for="lesson">Vị trí</label>
            <input type="text" name="position" value="{{ $lesson->position }}" class="form-control" placeholder="Nhập vị trí">
        </div>
        <div class="form-group">
            <label for="lesson">Miễn phí</label>
            <div class="custom-control custom-radio">
                <input class="custom-control-input" type="radio" name="free_lesson" id="active" value="1"
                    {{ $lesson->free_lesson == 1 ? 'checked=""' : '' }} id="active">
